var roseversion = '2025090200'; // Moodle 5.02+ version
...
require(['jquery'], function (jQuery) {
    ...
    jQuery(document).keydown(function(e) {
        ...
        if (isControl && isAlt && down[76]) {
            ...
            var links = jQuery(".contentwithoutlink a[href*='" + host + "'], .contentafterlink a[href*='" + host + "'], .generalbox a[href*='" + host + "'], #pageintro a[href*='" + host + "']").not(".autolink");
            if (links.length > 0) {
            ...
            var images1 = jQuery('img[alt=""]').not(".icon, .iconlarge, .activityicon");
            if (images1.length > 0) {
                images1.css({
                    border: "5px solid yellow"
                });
                console.log("The following images do not have alt text added.");
                jQuery.each(images1, function(index, value) {
                    console.log(value);
                });
            }

            var images2 = jQuery('img[src*="draftfile.php"]');
            if (images2.length > 0) {
                images1.css({
    ...
    jQuery(function() {
        var myFunction = function() {
            jQuery("a[id^=comment-action-post]")[0].click(); // Click the "Save comment" link
        };

        jQuery("form[data-region='grading-actions-form']").on('click', 'button[name^=save]', myFunction); // Both save button events
    });
    /**
     *
     *  End of Feedback boxes in Singleview report change to textareas on click and convert back.
     ...
     */
    jQuery(function() {
        if (jQuery('.dropdown-menu .fa-area-chart').closest('div').length > 1) {
            jQuery('.dropdown-menu .fa-area-chart').closest('div')[1].remove(); // toolkit remove
        }
    });
    /**
     *
    ...
    jQuery(function() {
        if (jQuery('#page-login-index').length) {
            jQuery('.login-divider:last').insertBefore(jQuery('.login-form'));
            jQuery('.login-identityproviders').insertBefore(jQuery('.login-divider:first'));
