var roseversion = '2024052800'; // Moodle 4.2.4+ version
 ...
 */
 ...
 return result;}
 function init(converter){function api(key,value,attributes){if(document===undefined){return;}
...
*
*/
function rosekeycode(keys, elem) {
...
*
*/
function waitFor(selector, callback, maxTimes = false) {
...
jQuery(document).ready(function($) {
    ...
    */
    ...
    function create_unicode_button_set(name, set) {
        if ($('*[data-fieldtype="editor"]').length > 0 || $('.qtype_essay_response').length > 0 || $('.answer input').length > 0 || $("input[name*='answer[']").length > 0) {
            if ($('.' + name).length > 0) { // if it is already showing, remove it.
                ...
                $('*[data-fieldtype="editor"], .qtype_essay_response, span.answer input, .d-flex:has(input[name*="answer["])').after("<div class='unicodehelper " + name + " col-md-3'></div><div class='unicodehelper " + name + " col-md-9 felement'></div>");
    ...
    $(document).keydown(function(e) {
      ...
      if(down[91] || down[224] || down[17]) { isControl = true; }
      if(down[18]) { isAlt = true; }
        ...
        if (isControl && isAlt && down[68]) {
            var items =
            ['&aacute;','&eacute;','&iacute;','&oacute;','&uacute;','&ntilde;','&uuml;','&iexcl;','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;','&Ntilde;','&Uuml;','&iquest;'];
        ...
        if (isControl && isAlt && down[76]) {
            console.clear();
            var links = $(".contentwithoutlink a[href*='rose-hulman'], .contentafterlink a[href*='rose-hulman'], .generalbox a[href*='rose-hulman'], #pageintro a[href*='rose-hulman']").not(".autolink");
            if(links.length > 0) {
            ...
            if(images1.length > 0) {
                images1.css({border: "5px solid yellow"});
                console.log("The following images should be checked.");
            ...
            if(images2.length > 0) {
                images1.css({border: "8px solid red"});
                console.log("The following images must be checked.");
                $.each(images1, function(index, value){
                    console.log(value);
                });
            }
        ...
        if (isControl && isAlt && down[80]) {
            if($('.navbar').is(":hidden")) {
                // Show Everything.
                $('.navbar').show(); $('#page').show(); $('#page-footer').show();
                $('#printable_quiz').remove();
            } else {
                // Hide everything.
                $('.navbar').hide(); $('#page').hide(); $('#page-footer').hide();
        ...
        */
        if (isControl && isAlt && down[67]) {
            if (Cookies.get("rhhidebrickfieldicon") != null) {
                ...
                $('.fa-retweet').parent('a').html(function (_, ctx) { return ctx.replace("Show AltFormat", "Hide AltFormat"); });
            } else {
                Cookies.set("rhhidebrickfieldicon", true, { expires: 365 });
                $(".local-bfaltformat-access-button").parent("span").hide();
                $('.fa-retweet').parent('a').html(function (_, ctx) { return ctx.replace("Hide AltFormat", "Show AltFormat"); });
            }
        }

        for (var keycodes in down){ delete down[keycodes]; }
        isControl = false;
        isAlt = false;
        e.preventDefault();
    });
    ...
    if (Cookies.length) {
        if (Cookies.get("scroll") !== null && Cookies.get("oldlocation") !== null && Cookies.get("oldlocation") == newlocation) {
            ...
            if(params.search(pageediting) != -1){ // Page editing mode
                var scrollextras = {x: .12, f: .0135}; // 12% page length difference in editing mode and .0135 fudge factor
                ...
                var scrollextras = {x: .02, f: 0}; // 2% page length difference in editing mode and 0 fudge factor
                multiplier = multiplier + scrollextras['x'];
                Cookies.set("scrollextras", scrollextras);
 ...
 */
    $(function() {
        ...
        if(surveyTitle == "My Colorful Portrait"){
            setInterval(
                function(fieldsets){
                    var colorGroups = {"orange": 0, "gold": 0, "blue": 0, "green": 0};
 ...
 */
$(function() {
	setInterval(createflexiblevideos, 1000);
		 ...
		 $("iframe, embed, object").not('.sketch_iframe').not("div[data-fieldtype='editor'] iframe", "div[data-fieldtype='editor'] embed", "div[data-fieldtype='editor'] object").not(".panopto-flexible-iframe").each(function (index, value) {
			  ...
			  if (url !== false) {
					...
					if(value.type !== undefined && value.type.indexOf("flash") > 0) {
					  if(url.indexOf("youtube") > 0 || url.indexOf("youtu.be") > 0) { // youtube embed.
							$(this).removeAttr("type");
							var clone = $(this).clone();
							$(this).after(clone);
							$(this).remove();
					  }
					  if(url.indexOf("panopto") > 0) { // panopto embed.
							$(this).removeAttr("type");
							var clone = $(this).clone();
							$(this).after(clone);
				 ...
				 if(url.indexOf("/lib/editor/tiny/plugins/panoptoltibutton/view") > 0 || // Panopto TinyMCE LTI.
					 url.indexOf("/lib/editor/atto/plugins/panoptoltibutton/view") > 0 || // Panopto Atto LTI.
					 url.indexOf("panopto.com/Panopto/Pages/Embed") > 0 || // Panopto original embed.
					 url.indexOf("youtube") > 0 || url.indexOf("youtu.be") > 0) { // Youtube embed.
					  $(this).removeAttr("width");
					  $(this).removeAttr("height");
					  $(this).addClass("panopto-flexible-iframe");
					  $(this).wrap("<div>");
					  $(this).parent("div").addClass("panopto-flexible-container");
				 }	
			  }
		 })
	}
