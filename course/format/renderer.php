...
abstract class format_section_renderer_base extends plugin_renderer_base {
    ...
    protected function section_edit_control_menu($controls, $course, $section) {
        ...
        if (!empty($controls)) {
            ...
            if ($section->section && get_string_manager()->string_exists('sectionactions',
                                                                            'format_'.$course->format)) {
                $menu->set_menu_trigger(get_string('sectionactions', 'format_'.$course->format));
            } else {
                $menu->set_menu_trigger(get_string('sectionactions'));
            }
            ...
            foreach ($controls as $value) {
                ...
                $al = new action_menu_link_secondary(new moodle_url($url),
                                                     new pix_icon($icon, $name, null, array("class" => $class,
                                                                                            "alt" => $alt)
                                                     ), $name, $attr
                );
                $menu->add($al);
     ...
     * @deprecated since Moodle 2.9 MDL-48947 - please do not use this function any more.
    ...
    protected function section_edit_controls($course, $section, $onsectionpage = false) {
        ...
        foreach ($items as $item) {
                ...
                $class = empty($item["pixattr"]["class"]) ? "" : $item["pixattr"]["class"];
                $alt = empty($item["pixattr"]["alt"]) ? "" : $item["pixattr"]["alt"];
                $controls[] = html_writer::link(new moodle_url($url),
                        html_writer::empty_tag('img', array('src' => $this->output->pix_url($icon),
                        'class' => $class, 'alt' => $alt)),
                        $attr);
        }
    ...
    protected function section_edit_control_items($course, $section, $onsectionpage = false) {
        ...
        if (!$onsectionpage) {
            if (!$isstealth && has_capability('moodle/course:update', $coursecontext)) {
                ...
                $controls[] = array('url' => new moodle_url('/course/editsection.php',
                                                            array('id' => $section->id, 'sr' => $onsectionpage)),
                                    'icon' => 'i/settings',
                                    'name' => $streditsection,
                                    'pixattr' => array('class' => 'iconsmall', 'alt' => $streditsection),
            ...
            if ($section->section) {
                ...
                if (!$isstealth) {
                    if (has_capability('moodle/course:sectionvisibility', $coursecontext)) {
                        if ($section->visible) { // Show the hide/show eye.
                            ...
                            $controls[] = array('url' => $url, 'icon' => 'i/hide', 'name' => $strhidefromothers,
                            ...
                            $controls[] = array('url' => $url, 'icon' => 'i/show', 'name' => $strshowfromothers,
                    ...
                    if (has_capability('moodle/course:movesections', $coursecontext)) {
                        ...
                        if ($section->section > 1) { // Add a arrow to move section up.
                            ...
                            $controls[] = array('url' => $url, 'icon' => 'i/up', 'name' => $strmoveup,
                        ...
                        if ($section->section < $course->numsections) { // Add a arrow to move section down.
                            ...
                            $controls[] = array('url' => $url, 'icon' => 'i/down', 'name' => $strmovedown,
                ...
                if (course_can_delete_section($course, $section)) {
                    ...
                    $controls[] = array('url' => $url, 'icon' => 'i/delete', 'name' => $strdelete,
